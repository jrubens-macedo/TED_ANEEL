...
nome_arquivo = r'C:\pythonjr\tedaneel\3013983346.csv'
...
# Filtra os registros associados a dias úteis
    ...
    plt.text(0.183, 0.88, f'Máx kW (DU) = {max_kW_DU:.2f} kW', transform=plt.gcf().transFigure,
             ...
             bbox=dict(facecolor='white', edgecolor='white'), fontweight='bold', fontsize=11)
    plt.text(0.183, 0.84, f'(ANUAL) UGMT = {mes_ugmt}', transform=plt.gcf().transFigure,
             horizontalalignment='center', verticalalignment='center',
             bbox=dict(facecolor='white', edgecolor='white'), fontweight='bold', fontsize=11)

    plt.title('ANÁLISE DIAS ÚTEIS (DU)', fontsize=15)
    plt.xlabel('Hora do Dia', fontsize=14)
    plt.ylabel('Potência Ativa (pu)', fontsize=14)
    ...
    plt.text(0.514, 0.88, f'Máx kW (SA) = {max_kW_SA:.2f} kW', transform=plt.gcf().transFigure,
             horizontalalignment='center', verticalalignment='center',
             bbox=dict(facecolor='white', edgecolor='white'), fontweight='bold', fontsize=11)
    plt.text(0.514, 0.84, f'(ANUAL) UGMT = {mes_ugmt}', transform=plt.gcf().transFigure,
             horizontalalignment='center', verticalalignment='center',
             bbox=dict(facecolor='white', edgecolor='white'), fontweight='bold', fontsize=11)

    plt.title('ANÁLISE SÁBADOS (SA)', fontsize=15)
    plt.xlabel('Hora do Dia', fontsize=14)
    plt.ylabel('Potência Ativa (pu)', fontsize=14)
    plt.grid(True, linestyle='dotted', alpha=0.7)
    plt.ylim(0, 1.2)
    ...
    plt.text(0.8434, 0.88, f'Máx kW (DO) = {max_kW_DO:.2f} kW', transform=plt.gcf().transFigure,
             ...
             bbox=dict(facecolor='white', edgecolor='white'), fontweight='bold', fontsize=11)
    plt.text(0.8434, 0.84, f'(ANUAL) UGMT = {mes_ugmt}', transform=plt.gcf().transFigure,
             horizontalalignment='center', verticalalignment='center',
             bbox=dict(facecolor='white', edgecolor='white'), fontweight='bold', fontsize=11)

    plt.title('ANÁLISE DOMINGOS (DO)', fontsize=15)
    plt.xlabel('Hora do Dia', fontsize=14)
    plt.ylabel('Potência Ativa (pu)', fontsize=14)
    plt.grid(True, linestyle='dotted', alpha=0.7)
    plt.ylim(0, 1.2)
